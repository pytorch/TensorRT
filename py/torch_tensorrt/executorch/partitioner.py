...
class TensorRTPartitioner(Partitioner):  # type: ignore[misc]
    ...
    def _partition_elided_output_names(
        ...
        An extraction failure is survivable only for a method in which nothing was
        rewired: the delegate then really does carry every binding, and a genuinely
        missing aliased output stays an error in the backend's
        ``_validate_output_binding_order``. Where a buffer *was* rewired the aliased
        outputs are already gone from the graph -- that happened before partitioning
        -- so an empty set stamps nothing, and the export dies downstream blaming a
        lost aliased-buffer mark, which is not what went wrong. That case re-raises.

        A set that is neither empty nor the engine's whole ``aliased_io`` is the
        right answer and still not a lowerable one, since the runtime reads
        elision off a single argument count: ``TensorRTBackend.preprocess``
        refuses that engine.
        """
        ...
        try:
            ...
            # Broad in the same shape as _resolve_target_device_for_partition, but
            # not with the same licence: that fallback picks a default device and
            # is genuinely harmless, while this one can only be harmless where no
            # output was elided. The question is asked of the graph, not of the
            # engine metadata that just failed to read, so it cannot fail the same
            # way.
            if any(
                node.op == "placeholder"
            ...
            logger.warning(
                ...
                "(%s); no buffer in this method was rewired, so the delegate "
                "carries every binding.",
                getattr(partition, "id", "?"),
                e,
            )
            return set()
