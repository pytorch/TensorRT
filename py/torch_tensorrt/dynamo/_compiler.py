...
def compile(
    ...
    *,
    ...
    use_strong_typing: bool = _defaults.USE_STRONG_TYPING,
    ...
    **kwargs: Any,
    ...
    Keyword Arguments:
        ...
        use_strong_typing (bool): Enable strong typing in TensorRT compilation
    ...
    if use_strong_typing:
        if len(enabled_precisions) != 1 or not any(
            x in enabled_precisions for x in {torch.float32, dtype.f32}
        ):
            raise AssertionError(
                f"When use_strong_typing is enabled, only torch.float32 is allowed in the enabled_precisions but found {enabled_precisions}"
            )
    ...
    compilation_options = {
        ...
        "use_strong_typing": use_strong_typing,
