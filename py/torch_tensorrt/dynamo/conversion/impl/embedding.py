...
def embedding_bag(
    ...
    if include_last_offset:
        ...
        offsets = impl.cat.cat(
            ...
            source_ir,
            f"{name}_concat",
            [offsets, torch.tensor([indices.shape[0]], dtype=torch.int32)],
            0,
        )

    reduced_embed = []
    start = ctx.net.add_gather(
        offsets, get_trt_tensor(ctx, 0, "tensor_0"), 0
    ).get_output(0)
    const_list = get_trt_tensor(ctx, np.array(range(indices.shape[0])), "constant_list")

    for i in range(1, offsets.shape[0]):
        end = ctx.net.add_gather(
            offsets, get_trt_tensor(ctx, i, f"tensor_{i}"), 0
        ).get_output(0)
        diff = impl.elementwise.sub(
            ctx, target, source_ir, f"{name}_sub_{i}", end, start
        )

        slice_layer = ctx.net.add_slice(const_list, (1,), (1,), (1,))
        slice_layer.set_input(1, start)
        slice_layer.set_input(2, diff)
        sliced_indices = slice_layer.get_output(0)
...
def embedding_bag(
    ...
    reduced_embed = []
    start = ctx.net.add_gather(offsets, get_trt_tensor(ctx, 0, f"tensor_0"), 0).get_output(0)
    const_list = get_trt_tensor(ctx, np.array(range(indices.shape[0])), f"constant_list")

    for i in range(1, offsets.shape[0]):
        end = ctx.net.add_gather(offsets, get_trt_tensor(ctx, i, f"tensor_{i}"), 0).get_output(0)
        diff = impl.elementwise.sub(ctx, target, source_ir, f"{name}_sub_{i}", end, start)
        ...
        index_embed = impl.select.index(ctx, target, source_ir, f"{name}_index_{i}", embed, [sliced_indices])  # np.array([0,1,2,3,4,5])
        reduced_sliced_embed = reduce_op(
            name=f"{name}_{reduce_name}_{i}",
            input_val=index_embed,
            dim=0,
