...
class FakeTensorUpdater:
    ...
    def incremental_update(self, fake_mode: FakeTensorMode) -> None:
        ...
        def should_process_node(node: torch.fx.Node) -> bool:
            ...
            # tensors from an op.
            _generalized_scatter = getattr(
                getattr(
                    getattr(torch._inductor, "fx_passes", None),
                    "reinplace",
                    None,
                ),
            ...
            try:
                if (shape_env := fake_mode.shape_env) and (
                    symbol_to_path := compute_unbacked_bindings(
                        shape_env, new_fake_tensor
                    )
                ):
                    # Refresh the bindings to the new symbols
                    node.meta["unbacked_bindings"] = symbol_to_path
            except Exception:
                # compute_unbacked_bindings may raise PendingUnbackedSymbolNotFound
                # when unbacked SymInts are used as scalar inputs (e.g. slice indices)
                # rather than embedded in output tensor shapes.  This is safe to ignore
                # since the metadata (val) has already been set above.
                pass
