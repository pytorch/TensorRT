...
the low-level functional attention operators. The interface is designed to provide a homogenous
...
class SequenceInfo:
    ...
    input_ids: torch.Tensor = field(
        default_factory=lambda: torch.zeros(1, 1, dtype=torch.int)
    )
    seq_len: torch.Tensor = field(
        default_factory=lambda: torch.ones(1, dtype=torch.int)
    )
    input_pos: torch.Tensor = field(
        default_factory=lambda: torch.zeros(1, dtype=torch.int)
    )
    cache_loc: torch.Tensor = field(
        default_factory=lambda: torch.arange(1, dtype=torch.int)
    )
    pages_per_seq: torch.Tensor = field(
        default_factory=lambda: torch.ones(1, dtype=torch.int)
    )
    ################################################################################################
    ...
    def __post_init__(self):
        ...
        self._num_pages = (total_tokens) // self.page_size + (
            total_tokens % self.page_size > 0
        )
    ...
    def page_assignments(self) -> List[List[int]]:
        ...
        return [
            ...
            for c_loc_one_seq in torch.split(
                self.cache_loc[: sum(pages_per_seq)], pages_per_seq
            )
        ]

    @classmethod
    def _get_sanitized_seq_len(
        cls, input_ids: torch.Tensor, seq_len: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _get_sanitized_num_sequences(
        input_ids: torch.Tensor, seq_len: torch.Tensor
    ) -> int:
    ...
    def sync(self, other: "SequenceInfo") -> None:
        for f in fields(self):
            ...
            if f.name == "input_ids":
                ...
                assert (
                    val == val_other
                ), f"Field {f.name} mismatch: {val} != {val_other}."
    ...
    def reset(self) -> None:
        ...
        self.cache_loc[:] = torch.arange(
            self.num_pages, dtype=torch.int, device=self.device
        )
    ...
    def update_pos(
        self, seq_len: Union[torch.Tensor, List[int], int], reset: bool = False
    ) -> None:
    ...
    def assign_cache_loc(self, page_assignments: Sequence[Sequence[int]]) -> None:
        ...
        pages_per_seq = torch.tensor(
            [len(p) for p in page_assignments], dtype=torch.int
        )
...
class AttentionDescriptor(ABC):
    ...
    def get_prepare_metadata_op(cls) -> Tuple[PrepareMetadataCallable, int]:
        ...
        on to each invokation of the attention op in the graph.
    ...
    @classmethod
    def get_global_buffer_initializers(
        cls, attention_info: AttentionInfo
...
class AttentionRegistry:
    ...
    def register(cls, kernel_source: str) -> Type["AttentionDescriptor"]:
        def decorator(attention_cls: Type["AttentionDescriptor"]):
            assert (
                kernel_source not in cls._attention_registry
    ...
    def get(cls, kernel_source: str) -> Type["AttentionDescriptor"]:
        assert cls.has(
            kernel_source
        ), f"Attention source {kernel_source} not registered."
...
class CachedSequenceInterface:
    ...
    def __init__(
        self,
        attention_op: Type[AttentionDescriptor],
        sequence_info: SequenceInfo,
        device: Optional[Union[str, torch.device, int]] = None,
    ) -> None:
        if sequence_info.is_paged:
            assert (
                attention_op.is_paged()
            ), "Paged sequence info requires paged attention op."
    ...
    def initialize_caches(self) -> None:
        ...
        self._caches = {
            name: get_cache(self.info)
            for name, get_cache in self._cache_initializers.items()
        }
