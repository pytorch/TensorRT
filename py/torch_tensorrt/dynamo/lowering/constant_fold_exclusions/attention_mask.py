...
def _find_attention_mask_aranges(
    ...
    """Find INT32/INT64 position aranges feeding an attention mask.

    Other range dtypes, including unknown dtypes, retain normal constant
    folding. Integer ranges also remain foldable when mask construction uses
    FP64 intermediates: TensorRT cannot represent those tensors, and truncating
    them can change position comparisons.
    """
    nodes_to_visit: list[torch.fx.Node] = [mask_node]
    ...
    while nodes_to_visit:
        ...
        dtype = getattr(value, "dtype", current.kwargs.get("dtype"))
        if dtype is torch.float64:
            # Leave the whole constructor foldable, including integer aranges
            # compared with FP64 values or cast to FP64 after construction.
            return []

        if _is_arange_node(current):
            if dtype not in (torch.int32, torch.int64):
                # Keep mixed-dtype constructors foldable as a whole, rather
                # than preserving only their integer ranges.
                return []
