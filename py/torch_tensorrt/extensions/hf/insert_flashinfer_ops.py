...
from torch_tensorrt.extensions.hf import AttentionInfo, kv_cache_manager
...
def insert_flashinfer_attn_with_cache(
    ...
    for mha_node in graph.nodes:
        ...
        num_kv_heads = mha_gemms[mha_node][1][1].meta["val"].shape[1]

        mha_info[mha_node] = AttentionInfo(
            num_heads=num_q_heads,
            num_kv_heads=num_kv_heads,  # kv_gemm_fake.shape[-1] // q_fake.shape[3],
            head_dim=head_dim,
            dtype=q_fake.dtype,
            cache_dtype=None,
            rope_theta=500000,
            sm_scale=mha_node.kwargs.get("scale", 1 / (head_dim**0.5)),
        )
    ...
    for idx, (mha_node, gemms) in enumerate(mha_gemms.items()):
        ...
        if "val" in mha_node.meta:
            mha_node_with_cache.meta["val"] = copy.copy(mha_node.meta["val"][0])

        # Renaming the node to avoid conflict with the next MHA node
        mha_node.name = f"{mha_node.name}_original"
        mha_node_with_cache.name = mha_node.name

        # Detect getitem node and remove it
        getitem_node = None
        if mha_node.users:
            for user in mha_node.users:
                if user.op == "call_function" and user.target == operator.getitem:
                    getitem_node = user
                    break
        if getitem_node:
            getitem_node.replace_all_uses_with(mha_node_with_cache)
