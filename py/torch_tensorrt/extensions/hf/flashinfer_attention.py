...
import flashinfer
import torch
from torch_tensorrt.extensions.hf.attention_interface import (
    AttentionDescriptor,
    AttentionInfo,
    AttentionRegistry,
...
# from .cuda_graph import cuda_graph_state
...
class _FlashInferPlanner:
    ...
    def plan(
        ...
        #     and plan_params not in self.cached_decode_wrappers
        # ):
        ...
        if (
            ...
            assert (
                plan_params.is_generate
            ), "Only generate is supported during cuda graph capture."
...
def prepare_flashinfer_metadata_fake(
    ...
    qo_indptr = torch.empty(
        len(seq_len) + 1, dtype=seq_len.dtype, device=seq_len.device
    )
    return (
        qo_indptr,  # qo_indptr
