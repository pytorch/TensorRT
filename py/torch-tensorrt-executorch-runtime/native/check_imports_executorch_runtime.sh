...
# The toolkit Bazel handed CMake can differ from torch.version.cuda, so check the linked major too.
...
if [ -n "${cuda_needed}" ]; then
    case "${cuda_needed}" in
        ...
        *)
            echo "FATAL: ${target} needs ${cuda_needed}, but this delegate requires CUDA 13." >&2
            exit 1
            ;;
    esac
