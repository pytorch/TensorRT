...
TEST_P(ModuleTests, SerializedModuleIsStillCorrect) {
    ...
    trt_mod.save("test_serialization_mod.ts");
    auto jit_mod = torch::jit::load("test_serialization_mod.ts");

    torch::jit::IValue jit_results_ivalues = trtorch::tests::util::RunModuleForward(jit_mod, jit_inputs_ivalues);
