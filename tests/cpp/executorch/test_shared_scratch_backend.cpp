...
bool cuda_device_is_required() {
  ...
  // Only "1" turns it on, matching every other flag in this project. Treating
  // anything non-empty as on would make REQUIRE_CUDA=false switch it on.
...
// The three cases below cover the rest of what a span can be. set_option reads the
// whole span before it stores anything, so a caller that batches its options needs
// to know what a repeat, a mixed span and an empty span do.

// Last one wins, which is what applying each entry in turn used to do and what the
// implementation's own comment promises. The two entries disagree, so a reader that
// stopped at the first would leave the setting on.
TEST_F(SharedScratchBackendTest, SetOptionTakesTheLastValueWhenASpanNamesTheKeyTwice) {
  ASSERT_EQ(set_shared_scratch(backend_, false), Error::Ok);

  BackendOption first;
  std::strncpy(first.key, kOptionKey, sizeof(first.key) - 1);
  first.value = true;
  BackendOption second;
  std::strncpy(second.key, kOptionKey, sizeof(second.key) - 1);
  second.value = false;
  BackendOption options[2] = {first, second};
  BackendOptionContext context;
  EXPECT_EQ(backend_.set_option(context, Span<BackendOption>(options, 2)), Error::Ok);

  LoadedEngine engine;
  ASSERT_EQ(engine.load(blob(), 5), Error::Ok);
  EXPECT_FALSE(engine.handle()->shared_scratch)
      << "the span named the key twice and the first value won, so the last entry was not the one stored";
}

// A foreign key ahead of this backend's own has to be passed over rather than
// ending the read, or the entry behind it never lands.
TEST_F(SharedScratchBackendTest, SetOptionReadsItsOwnKeyPastAForeignOneInTheSameSpan) {
  ASSERT_EQ(set_shared_scratch(backend_, false), Error::Ok);
  ...
  foreign.value = 7;
  BackendOption mine;
  std::strncpy(mine.key, kOptionKey, sizeof(mine.key) - 1);
  mine.value = true;
  BackendOption options[2] = {foreign, mine};
  BackendOptionContext context;
  EXPECT_EQ(backend_.set_option(context, Span<BackendOption>(options, 2)), Error::Ok);

  LoadedEngine engine;
  ASSERT_EQ(engine.load(blob(), 6), Error::Ok);
  EXPECT_TRUE(engine.handle()->shared_scratch)
      << "a foreign key ahead of this backend's own stopped the read, so the entry behind it was never stored";
}

// An empty span asks for nothing, so it is neither an error nor a reset.
TEST_F(SharedScratchBackendTest, SetOptionAcceptsAnEmptySpanAndChangesNothing) {
  ASSERT_EQ(set_shared_scratch(backend_, true), Error::Ok);

  BackendOptionContext context;
  EXPECT_EQ(backend_.set_option(context, Span<BackendOption>()), Error::Ok);

  LoadedEngine engine;
  ASSERT_EQ(engine.load(blob(), 7), Error::Ok);
  EXPECT_TRUE(engine.handle()->shared_scratch) << "an empty span moved the shared-scratch setting";
}
...
// Under the default Global mode the cudaMalloc that grows a host-input staging
// buffer, further down, is prohibited and invalidates the capture where it stands.
// A refusal after it is a clean error handed to a caller whose capture is already
// dead, which is the outcome the guard exists to prevent. Relaxed mode could not
// tell the two apart: it permits everything execute() does before it reaches the
// pool, so a guard sitting anywhere ahead of the pool would look the same.
//
// The first run leaves the pool holding a buffer with an enqueue recorded against
// it, so the handoff wait the refusal protects is one the run below would really
// have made. That wait is a cudaStreamWaitEvent on an event recorded outside the
// capture, which invalidates it under every capture mode.
...
TEST_F(SharedScratchBackendTest, ASecondPooledEnqueueWaitsForTheFirstOnAnotherStream) {
  ...
  //
  // The first call runs on its own thread because execute() waits for its engine
  // before returning, so on this thread it would block on the held stream and the
  // second call would never be made. On a thread of its own it gets as far as
  // recording its completion event and releasing the device's pool lock, then
  // blocks in its own wait, which is exactly the state this case needs. Waiting
  // for the pool to report a buffer is what says it claimed first: after that the
  // pool lock orders the two, since the second call cannot take it until the first
  // has recorded the event the handoff waits on.
  // Both calls run on threads of their own because execute() waits for its engine
  // before returning: the first would block on the held stream, and the second
  // blocks too, since the handoff puts its stream behind the first one's enqueue.
  // Each gets as far as recording its completion event and dropping the device's
  // pool lock before it blocks, which is the state this case needs. Waiting for the
  // pool to report a buffer is what says the first claimed first; after that the
  // pool lock orders the two, since the second cannot take it until the first has
  // recorded the event the handoff waits on.
  Error first_error = Error::Ok;
  Error second_error = Error::Ok;
  JoinAtScopeExit first_caller{std::thread([&] { first_error = first.run(first_stream); })};
  const auto claim_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
  while (shared_scratch_capacity_for_testing(first.handle()->device_id) == 0u &&
         std::chrono::steady_clock::now() < claim_deadline) {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
  const bool first_claimed = shared_scratch_capacity_for_testing(first.handle()->device_id) > 0u;
  JoinAtScopeExit second_caller{std::thread([&] { second_error = second.run(second_stream); })};

  // The second stream has to be seen carrying work before its completion means
  // anything: the call that queues it runs on a thread now, so an empty stream here
  // is a thread that has not got there yet, and cudaStreamQuery answers success for
  // that just as it does for finished work.
  bool second_stream_became_busy = false;
  const auto busy_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
  while (std::chrono::steady_clock::now() < busy_deadline) {
    if (cudaStreamQuery(second_stream) == cudaErrorNotReady) {
      second_stream_became_busy = true;
      break;
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
  ...
  while (second_stream_became_busy && std::chrono::steady_clock::now() < deadline) {
  ...
  const bool watchdog_opened_the_gate = gate.forced_open.load();

  // Before any assertion that can return early: both callers are blocked on this
  // gate, and the guards that join them are destroyed ahead of the one that opens
  // it, so an early return with the gate still held would join a thread that
  // cannot finish.
  gate_release.release();
  first_caller.join();
  second_caller.join();

  ASSERT_FALSE(watchdog_opened_the_gate)
      << "the watchdog had to open the gate, so nothing below was measured under the conditions it describes";
  ASSERT_TRUE(first_claimed)
      << "the first call never claimed a pooled buffer, so there is no handoff for the second to wait on";
  ASSERT_TRUE(second_stream_became_busy)
      << "the second call never queued anything on its stream, so its stream finishing says nothing about the handoff";
...
void SharedScratchBackendTest::run_a_growth_beside_parked_work(bool synchronized_path) {
      ...
      << "the pool did not grow, so no buffer was retired and nothing was disposed of";
  {
    ...
    // here whatever the disposal went on to do with it. The unit cases under
    // SharedScratchDisposalStream are what pin that; it is not observable from a
    // backend call's timing, because execute() synchronizes the caller's stream
...
TEST_F(SharedScratchBackendTest, AGrowthOnACallerStreamWaitsForTheEnqueueOnTheBufferItRetires) {
  ...
  // On its own thread: execute() waits for its engine before returning, so on this
  // thread it would block on the parked stream and the growth below would never be
  // made. It gets as far as recording its completion event and dropping the pool
  // lock before it blocks, which is the state the growth has to meet. The capacity
  // read is what says it claimed, and the pool lock orders the two after that.
  Error small_error = Error::Ok;
  JoinAtScopeExit small_caller{std::thread([&] { small_error = small.run(held); })};
  const auto claim_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
  while (shared_scratch_capacity_for_testing(device_id) == 0u && std::chrono::steady_clock::now() < claim_deadline) {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
  const std::size_t before = shared_scratch_capacity_for_testing(device_id);
  ASSERT_GT(before, 0u) << "the smaller engine left the pool empty, so the larger one has nothing to retire";
  ...
  EXPECT_EQ(small_error, Error::Ok);
  EXPECT_EQ(growth_error, Error::Ok);
...
// The drain above belongs to the pooled path. This case is its opposite number on
// the unpooled one, and both now hold: the runtime drains a staged input's copy on
// any error return past it, whether or not the engine draws on the pool. That is
// not what this delegate did when the pool was written, when the wait was scoped
// to pooled calls on the grounds that closing it for everyone would change what a
// default-off call does. It has since been closed for everyone, which is the
// better answer: the copy reads the caller's own memory, and the caller owns that
// memory again the moment execute() returns, so leaving it running was a hazard on
// either path.
//
...
// straight away.
TEST_F(SharedScratchBackendTest, AFailedUnpooledCallDrainsTheHostInputCopyItQueued) {
  cudaStream_t stream = nullptr;
  ASSERT_EQ(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking), cudaSuccess);
  ...
  // Short, because not returning is the expected outcome now and this window is
  // paid in full every run: the stream stays parked until this case releases it,
  // so an early return would land well inside a window this size.
  const auto give_up_waiting_at = std::chrono::steady_clock::now() + std::chrono::seconds(3);
  ...
  EXPECT_FALSE(returned_while_the_stream_was_parked)
