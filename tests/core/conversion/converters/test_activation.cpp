...
TEST(Converters, ATenGELUConvertsCorrectly) {
  ...
  // NOTE: The official tensorrt plugin applies the Gelu activation x * Phi(x), where Phi is the Gaussian cdf, approximated
  // by: 0.5 * (1 + tanh(sqrt(2 / M_PI) * (x + 0.044715 * x^3))) and the pytorch uses c10::cuda::compat::normcdf to
  // compute Phi(x). So there's a difference here and therefore the threshold is slightly higher than other ops.
  // One in ten runs will give you an out of normal threshold result
