...
def _cossim_complex(py_out: torch.Tensor, trt_out: torch.Tensor, tag: str) -> None:
    ...
    assert (
        trt_out.shape == py_out.shape
    ), f"{tag}: shape mismatch {trt_out.shape} vs {py_out.shape}"
    ...
    assert (
        r > COSINE_THRESHOLD
    ), f"{tag}: real part cosine sim {r:.4f} < {COSINE_THRESHOLD}"
    assert (
        i > COSINE_THRESHOLD
    ), f"{tag}: imag part cosine sim {i:.4f} < {COSINE_THRESHOLD}"
...
def test_complex_div():
    ...
    b = _make_freqs(
        8, 64, theta=500.0
    )  # different theta → different angles → non-trivial imaginary
    inputs = (a, b)
    trt_model = torchtrt.compile(model, inputs=inputs, **_COMPILE)
    py_out = model(*inputs)
    trt_out = trt_model(*inputs)
...
class ComplexStackRealView(nn.Module):
    ...
    def forward(
        ...
        return torch.view_as_real(a * c).flatten(-2) + torch.view_as_real(
            b * c
        ).flatten(-2)
...
class ComplexWhere(nn.Module):
    ...
    def forward(
        self, mask: torch.Tensor, a: torch.Tensor, b: torch.Tensor
    ) -> torch.Tensor:
...
def test_dual_complex_path():
    ...
    freqs = (
        _make_freqs(8, 64).unsqueeze(0).unsqueeze(2).expand(2, 8, 4, 32).contiguous()
    )
    inputs = (x, y, freqs)
    trt_model = torchtrt.compile(model, inputs=inputs, **_COMPILE)
    py_out = model(*inputs)
...
def test_complex_mul_add_sub():
    ...
    def _rc():
        return torch.polar(
            torch.rand(8, 16, device="cuda") + 0.2,
...
def test_complex_sin_cos_pythagorean():
    ...
    r = torch.rand(8, 16, device="cuda") * 0.4
    theta = (
        torch.linspace(0.1, 1.2, 16, device="cuda")
        .unsqueeze(0)
        .expand(8, -1)
        .contiguous()
    )
    ...
    trt_out = trt_model(z)
    _cossim_real(
        py_out.real.contiguous(),
        trt_out.real.contiguous(),
        "complex_sin_cos_pythagorean",
    )
    torch._dynamo.reset()
...
class ComplexNormalize(nn.Module):
    ...
    def forward(self, z):
        ...
        mag_c = torch.view_as_complex(torch.stack([mag, torch.zeros_like(mag)], dim=-1))
        return z / mag_c
...
def test_four_complex_inputs_mul_add():
    ...
    _cossim_complex(
        model(z1, z2, z3, z4), trt_model(z1, z2, z3, z4), "four_complex_inputs_mul_add"
    )
    torch._dynamo.reset()
