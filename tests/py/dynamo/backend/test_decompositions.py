...
def test_lowering_var_mean_correction(self):
    class var_mean(torch.nn.Module):
        def forward(self, x):
            return torch.var_mean(x, dim=[1], correction=2, unbiased=True)
...
def test_lowering_var_mean_correction_no_unbiased(self):
    class var_mean(torch.nn.Module):
        def forward(self, x):
            return torch.var_mean(x, dim=[1], correction=2)
