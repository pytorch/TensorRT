...
class TestLowering(TestCase):
    ...
    def test_lowering_rsub(self):
        class RSub(torch.nn.Module):
            def forward(self, x, y):
                return torch.rsub(x, y, alpha=5)
        ...
        unexpected_ops = {torch.ops.aten.rsub.default}
        ...
        fx_graph = torch.fx.symbolic_trace(RSub())
        unexpected_ops_seen, expected_ops_unseen = lower_graph_testing(
            fx_graph,
            inputs,
            expected_ops=expected_ops,
        ...
        self.assertAlmostEqual(
            ...
            0,
            DECIMALS_OF_AGREEMENT,
            f"Rsub TRT outputs don't match with the original model.",
        )
