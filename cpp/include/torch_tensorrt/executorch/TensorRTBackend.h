...
class TensorRTBackend final : public ::executorch::runtime::BackendInterface {
 public:
   ...
   * profile and CUDA stream the calling thread's guards asked for.
   */
...
/**
 * @brief Selects, for the calling thread, the CUDA stream the delegate runs
 * TensorRT on; scope it around execution.
 *
