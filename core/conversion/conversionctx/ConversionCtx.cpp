...
ConversionCtx::ConversionCtx(BuilderSettings build_settings)
	...
	auto nbDLACores = builder->getNbDLACores();
        TRTORCH_CHECK(settings.device.dla_core < nbDLACores, "Configured DLA Core ID: " << settings.device.dla_core << " not available. Total number of available DLA Cores: " << nbDLACores);
