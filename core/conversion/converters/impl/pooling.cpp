...
bool AdaptiveAvgPoolingConverter(ConversionCtx* ctx, const torch::jit::Node* n, args& args) {
  ...
  if (ctx->input_is_dynamic) {
    ...
    auto pooling_layer = ctx->net->addPoolingNd(*in, nvinfer1::PoolingType::kAVERAGE, util::toDims(window));
...
bool PoolingConverter(ConversionCtx* ctx, const torch::jit::Node* n, args& args, std::string pool_mode) {
  auto in = args[0].ITensorOrFreeze(ctx);
  ...
  if (pool_mode  == "Max") {
    ...
    new_layer = ctx->net->addPoolingNd(*in, nvinfer1::PoolingType::kMAX, kernel_size);
...
auto pooling_registrations TRTORCH_UNUSED =
    RegisterNodeConversionPatterns()
		...
		return PoolingConverter(ctx, n, args, "Max");
             }})
        .pattern(
            {"aten::avg_pool1d(Tensor self, int[1] kernel_size, int[1] stride=[], int[1] padding=0, bool ceil_mode=False, bool count_include_pad=True) -> Tensor",
             [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
		return PoolingConverter(ctx, n, args, "Avg");
             }})
	.pattern(
        ...
        .pattern(
            {"aten::adaptive_avg_pool2d(Tensor self, int[2] output_size) -> (Tensor)",
             [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
               return AdaptiveAvgPoolingConverter(ctx, n, args);
	      }})
        .pattern(
