            ...
            [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
              ...
              if (args[3].IValue()->isNone() && args[4].IValue()->isNone()) {
                ...
                f.emplace_back(nvinfer1::PluginField(
                    "scales", gamma.data.values, nvinfer1::PluginFieldType::kFLOAT32, gamma.data.count));
                f.emplace_back(nvinfer1::PluginField(
                    "bias", beta.data.values, nvinfer1::PluginFieldType::kFLOAT32, beta.data.count));

                fc.nbFields = f.size();
                fc.fields = f.data();

                nvinfer1::IPluginV2* pluginV2 = getPluginRegistry()
                                                    ->getPluginCreator("InstanceNormalization_TRT", "1", "trtorch")
                                                    ->createPlugin("InstanceNormalization_TRT", &fc);

                TRTORCH_CHECK(pluginV2, "Unable to create interpolation plugin from node" << *n);
                auto instance_norm_layer =
                    ctx->net->addPluginV2(reinterpret_cast<nvinfer1::ITensor* const*>(&input), 1, *pluginV2);
