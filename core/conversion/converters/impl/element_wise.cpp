...
nvinfer1::ILayer* add_elementwise(ConversionCtx* ctx,
    ...
    if (selfDim.size() != otherDim.size()) {
        if (otherDim.end() != std::find(otherDim.begin(), otherDim.end(), -1)) {
            auto otherShapeT = torch::ones(selfDim.size(), torch::kInt32);
            auto selfShapeM = torch::zeros(selfDim.size(), torch::kInt32);
            for (size_t start = selfDim.size() - otherDim.size(), idx = 0;
                 idx < otherDim.size(); ++idx) {
                if (-1 != otherDim[idx]) {
                    otherShapeT[start + idx] = otherDim[idx];
                } else {
                    otherShapeT[start + idx] = 0;
                    selfShapeM[start + idx] = 1;
                }
            }
            auto otherShapeTX = tensor_to_const(ctx, otherShapeT);
            auto selfShapeMX = tensor_to_const(ctx, selfShapeM);
            auto selfShape = ctx->net->addShape(*self)->getOutput(0);
            auto s1 =
                ctx->net
                    ->addElementWise(*selfShape, *selfShapeMX,
                                     nvinfer1::ElementWiseOperation::kPROD)
                    ->getOutput(0);
            auto s2 = ctx->net
                          ->addElementWise(*s1, *otherShapeTX,
                                           nvinfer1::ElementWiseOperation::kSUM)
                          ->getOutput(0);

            auto otherShuffle = ctx->net->addShuffle(*other);
            otherShuffle->setName(
                std::string(
                    "Reshape other tensor to have the same nDim as self for " +
                    name)
                    .c_str());
            otherShuffle->setInput(1, *s2);
            other = otherShuffle->getOutput(0);
        } else {
            auto otherShuffle = ctx->net->addShuffle(*other);
            otherShuffle->setReshapeDimensions(
                util::toDimsPad(otherDim, selfDim.size()));
            other = otherShuffle->getOutput(0);
        }
    }
    ...
    RegisterNodeConversionPatterns()
                ...
                args& args) -> bool {
                 ...
                 if (1 != scalar) {
                     ...
                     auto scaleLayer = ctx->net->addScale(
                         *other, nvinfer1::ScaleMode::kUNIFORM, unuse.data,
                         scaleW.data, unuse.data);
                     TRTORCH_CHECK(
                         scaleLayer,
                         "Unable to create scale layer from node: " << *n);
                     other = scaleLayer->getOutput(0);
                 }

                 auto add =
                     add_elementwise(ctx, nvinfer1::ElementWiseOperation::kSUM,
                                     self, other, util::node_info(n));
                 TRTORCH_CHECK(add,
                               "Unable to create add layer from node: " << *n);

                 add->setName(util::node_info(n).c_str());
                 auto out = ctx->AssociateValueAndTensor(n->outputs()[0],
                                                         add->getOutput(0));
            ...
            {"aten::sub.Tensor(Tensor self, Tensor other, Scalar alpha=1) -> "
             ...
             [](ConversionCtx* ctx, const torch::jit::Node* n,
                args& args) -> bool {
                 ...
                 auto sub =
                     add_elementwise(ctx, nvinfer1::ElementWiseOperation::kSUB,
                                     self, other, util::node_info(n));
                 TRTORCH_CHECK(sub,
                               "Unable to create sub layer from node: " << *n);
            ...
            {"aten::div.Tensor(Tensor self, Tensor other) -> Tensor",
             [](ConversionCtx* ctx, const torch::jit::Node* n,
                args& args) -> bool {
                 ...
                 auto other = args[1].ITensorOrFreeze(ctx);
                 auto div =
                     add_elementwise(ctx, nvinfer1::ElementWiseOperation::kDIV,
                                     self, other, util::node_info(n));

                 TRTORCH_CHECK(div,
                               "Unable to create div layer from node: " << *n);
            ...
            {"aten::mul.Tensor(Tensor self, Tensor other) -> Tensor",
             [](ConversionCtx* ctx, const torch::jit::Node* n,
                args& args) -> bool {
                 ...
                 auto other = args[1].ITensorOrFreeze(ctx);
                 auto mul =
                     add_elementwise(ctx, nvinfer1::ElementWiseOperation::kPROD,
                                     self, other, util::node_info(n));
                 TRTORCH_CHECK(mul,
                               "Unable to create mul layer from node: " << *n);
        ...
        .pattern({
            ...
            [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
                ...
                auto pow = add_elementwise(ctx, nvinfer1::ElementWiseOperation::kPOW,
                                           self, exponent, util::node_info(n));
