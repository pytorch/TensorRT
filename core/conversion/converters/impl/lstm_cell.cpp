...
auto lstm_cell_registrations TRTORCH_UNUSED = RegisterNodeConversionPatterns().pattern(
    {"aten::gru_cell(Tensor input, Tensor hx, Tensor w_ih, Tensor w_hh, Tensor? b_ih=None, Tensor? b_hh=None) -> Tensor",
     [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
       ...
       TRTORCH_CHECK(add_r, "Unable to create ElementWise layer from node: " << *n);
       auto activ_r = ctx->net->addActivation(*add_r->getOutput(0), nvinfer1::ActivationType::kSIGMOID);
       TRTORCH_CHECK(activ_r, "Unable to create sigmoid activation layer from node: " << *n);
       auto r_gate = activ_r->getOutput(0);

       // compute the Z gate
       auto add_z = ctx->net->addElementWise(*out1_z, *out2_z, nvinfer1::ElementWiseOperation::kSUM);
       TRTORCH_CHECK(add_z, "Unable to create ElementWise layer from node: " << *n);
       auto activ_z = ctx->net->addActivation(*add_z->getOutput(0), nvinfer1::ActivationType::kSIGMOID);
       TRTORCH_CHECK(activ_z, "Unable to create sigmoid activation layer from node: " << *n);
       auto z_gate = activ_z->getOutput(0);

       // compute the N gate
       auto out2_n_transform = ctx->net->addElementWise(*r_gate, *out2_n, nvinfer1::ElementWiseOperation::kPROD);
       TRTORCH_CHECK(out2_n_transform, "Unable to create ElementWise layer from node: " << *n);
       auto add_n = ctx->net->addElementWise(*out1_n, *out2_n_transform->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);
       TRTORCH_CHECK(add_n, "Unable to create ElementWise layer from node: " << *n);
       auto activ_n = ctx->net->addActivation(*add_n->getOutput(0), nvinfer1::ActivationType::kTANH);
       TRTORCH_CHECK(activ_n, "Unable to create tanh activation layer from node: " << *n);
       ...
       auto h_prime = ctx->net->addElementWise(*interm_2->getOutput(0), *interm_3->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);
       TRTORCH_CHECK(h_prime, "Unable to create ElementWise layer from node: " << *n);
