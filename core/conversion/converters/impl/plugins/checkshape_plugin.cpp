...
inline bool CheckShapePlugin::supportsFormatCombination(
  ...
  assert(
      pos < (nbInputs + nbOutputs) && nbInputs == CHECKSHAPE_PLUGIN_NUM_INPUT &&
      nbOutputs == CHECKSHAPE_PLUGIN_NUM_OUTPUT);
  ...
  for (int i = expandRank - 1; i >= 0; i--) {
    ...
    if (index_in >= 0) {
      if (h_expandShape[i] != -1) {
        if (h_inShape[i] != 1) {
          TRTORCH_CHECK(
              h_expandShape[i] == h_inShape[index_in],
              "The expanded size of the tensor (" << h_expandShape[i] << ") must match the existing size ("
      ...
      TRTORCH_CHECK(
          h_expandShape[i] >= 0,
          "The expanded size of the tensor (" << h_expandShape[i]
