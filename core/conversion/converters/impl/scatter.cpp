...
auto index_put_registrations TORCHTRT_UNUSED = RegisterNodeConversionPatterns().pattern(
     ...
     [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
       ...
       for (size_t i = 0; i < indices.size(); i++) {
         ...
         if (util::broadcastable(final_dim, cur_indice->getDimensions(), /*multidirectional=*/true)) {
           final_dim = util::broadcast(final_dim, cur_indice->getDimensions());
         } else {
           TORCHTRT_CHECK(true, "indices or update are not broadcastable");
         }
       ...
       for (size_t i = 0; i < indices_casted.size(); i++) {
         ...
         auto expand_layer = add_elementwise(
             ctx, nvinfer1::ElementWiseOperation::kPROD, indices_casted[i], const_out, "expand" + std::to_string(i));
       ...
       if (!accumulate) {
         ...
         auto scatterLayer =
             ctx->net->addScatter(*in, *concate_indices, *update_accumulate_out, nvinfer1::ScatterMode::kND);
         TORCHTRT_CHECK(scatterLayer, "Unable to create scatter nd layer from node: " << *n);
         auto out = ctx->AssociateValueAndTensor(n->outputs()[0], scatterLayer->getOutput(0));
         LOG_DEBUG("Output tensor shape: " << out->getDimensions());
       }
