...
via the TensorRT Model Optmiization SDK. It aims to provide better
...
This provides users the most runtime flexiblity, however limits options regarding serialization.
...
or select "High-level Ops" ammenable to TensoRT acceleration. Subgraphs are further partitioned into components that will run in PyTorch and ones to be further compiled to TensorRT based
...
Graphs go through the ``torch.export.trace`` system to be lowered into a graph consisting of `Core ATen Operators <https://pytorch.org/executorch/stable/ir-ops-set-definition.html>`_ or select "High-level Ops" ammenable to TensoRT acceleration.
...
with the Torch-TensorRT PyTorch runtime extention.
...
As there are many different frontends and supported formats, we provide a convience layer called ``torch_tensorrt.compile`` which lets users access
