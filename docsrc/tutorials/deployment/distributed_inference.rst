...
* The engine is serialized with ``is_md=True``, which tells the C++ runtime to bind
...
3. **Serialization** — the engine is serialized with the ``is_md=True`` flag in the
