...
def insert_kv_slicing_before_sdpa(gm, incoming_keys_values: List[Tuple[torch.Tensor, torch.Tensor]], start_idx_input: Node, end_idx_input: Node, is_causal_input: Node):
    ...
    kv_cache_for_graph = []
    for idx, sdpa_causal_true_node in enumerate(sdpa_nodes):
        assert len(sdpa_causal_true_node.args) == 6, f"SDPA node should have 6 arguments but got {len(sdpa_causal_true_node.args)} arguments"
        q_node, k_node, v_node, attn_mask, dropout_p, is_causal = sdpa_causal_true_node.args
        ...
        for key_or_value, current_key_or_value_node in zip([incoming_key, incoming_value], [k_node, v_node]):
            ...
            with gm.graph.inserting_before(sdpa_causal_true_node):
        ...
        # Add the new KV cache nodes as inputs to the SDPA node
        sdpa_causal_true_node.args = (q_node, new_keys_values[0], new_keys_values[1]) + (attn_mask, dropout_p, True)
        sdpa_causal_true_node.name = sdpa_causal_true_node.name + "_causal_true"

        # Add a new SDPA node with is_causal=False  
        with gm.graph.inserting_after(sdpa_causal_true_node):
            sdpa_causal_false_node = gm.graph.create_node(
                    op=sdpa_causal_true_node.op,
                    target=sdpa_causal_true_node.target,
                    args=sdpa_causal_true_node.args,
                    kwargs=sdpa_causal_true_node.kwargs,
                    name=sdpa_causal_true_node.name + "_causal_false",
                )
            sdpa_causal_false_node.args = sdpa_causal_false_node.args[:-1] + (False,)

        with gm.graph.inserting_after(sdpa_causal_false_node):
            # Add a torch.cond op which selects between two SDPA nodes ( one with is_causal=True and one with is_causal=False)
            cond_node_args = (is_causal_input, sdpa_causal_true_node, sdpa_causal_false_node)
            cond_node = gm.graph.create_node(
                    "call_function",
                    torch.ops.higher_order.cond,
                    args=(),
                )
            sdpa_causal_true_node.replace_all_uses_with(cond_node)
            cond_node.args = cond_node_args
    
    return gm, kv_cache_for_graph


@_aten_lowering_pass
def insert_static_cache_v3(
