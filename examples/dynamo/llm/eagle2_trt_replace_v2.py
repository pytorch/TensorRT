...
def run_benchmark(isl=2048, osl=128, device="cuda:1"):
    ...
    inputs = processor(text=txt, images=img_in, videos=vid_in,
                  return_tensors="pt", padding=True).to(device)

    # Validate ISL (optional)
    assert inputs["input_ids"].shape[1] == isl, (
    ...
    seq_tokens, step_times, overall_time, vision_time, mlp_time = generate_mm_with_timing(
        trt_model, inputs["pixel_values"], inputs["input_ids"],
        processor.tokenizer.eos_token_id, emb_layer, max_new_tokens=osl)
