...
def generate_reference(model, tokenizer, prompt, max_new_tokens=20):
    ...
    print(f"DEBUG: Using repetition_penalty={repetition_penalty} for Reference Generation")
...
def verify_output(trt_model_func, model_pytorch, tokenizer, prompt, max_new_tokens=20):
    ...
    with torch.no_grad():
        pyt_outputs = generate_reference(model_pytorch, tokenizer, prompt, max_new_tokens=30)
    ...
    pyt_outputs_generate_text = tokenizer.decode(pyt_outputs_generate_ids[0], skip_special_tokens=True)
    ...
    repetition_penalty = getattr(model_pytorch.generation_config, "repetition_penalty", 1.0)
    print(f"DEBUG: Using repetition_penalty={repetition_penalty} for TensorRT Generation")
    ...
    logits, kv_caches_delta = trt_model_func(input_ids, position_ids, kv_caches, ctx_len)
    ...
    if next_token.item() != tokenizer.eos_token_id:
        for _ in range(max_new_tokens - 1):
            ...
            logits, kv_caches_delta = trt_model_func(input_ids_step, position_ids_step, kv_caches, ctx_len_step)
...
if __name__ == "__main__":
    ...
    verify_output(trt_model_func, model_pytorch, tokenizer, "What is parallel programming?", max_new_tokens=30)
