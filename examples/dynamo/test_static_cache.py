...
def test_static_cache_model(args):
    ...
    with torch.inference_mode():
        ...
        # print_diff(out_no_cache, out_static_cache, "Prefill")
        torch.allclose(out_no_cache, out_static_cache, atol=ATOL, rtol=RTOL)
        ...
        for start_idx in range(2048, 2176):
            ...
            # print_diff(out_no_cache[:, :, -1:, :], out_static_cache, f"Generate {start_idx}")
...
def test_static_cache_lowering(args):
    ...
    # Test Prefill with torch_tensorrt
    q = torch.randn(1, 32, 2, 64).cuda()
    exported_program = export(
        model_no_cache,
        args=(q, k, v),
    ...
    print_diff(out_no_cache, out_trt, "Prefill TRT")
    breakpoint()
    # print_diff(trt_key_cache[:, :, :end_idx, :], k, "Prefill TRT key_cache")
    # print_diff(trt_value_cache[:, :, :end_idx, :], v, "Prefill TRT value_cache")
    assert torch.allclose(out_no_cache, out_trt, atol=ATOL, rtol=RTOL)
    breakpoint()
    ...
    for start_idx in range(2048, 2176):
        ...
        is_causal = False
        out_static_cache, pyt_key_cache, pyt_value_cache = model_static_cache(q_curr, k_curr, v_curr, pyt_key_cache, pyt_value_cache, start_idx, end_idx, is_causal)
        ...
        assert torch.allclose(out_static_cache, out_trt, atol=ATOL, rtol=RTOL)
        
        print_diff(out_static_cache, out_trt, f"Generate TRT {start_idx}")
        # breakpoint()
        assert torch.allclose(pyt_key_cache, trt_key_cache, atol=ATOL, rtol=RTOL)
        assert torch.allclose(pyt_value_cache, trt_value_cache, atol=ATOL, rtol=RTOL)
...
#     is_causal = True
#     out_no_cache = model_no_cache(q, k, v)
#     out_static_cache, pyt_key_cache, pyt_value_cache = model_static_cache(q, k, v, key_cache, value_cache, start_idx, end_idx, is_causal=True)
...
#         trt_model = torch_tensorrt.dynamo.compile(
#             exported_program,
#             inputs=[q, k, v, key_cache, value_cache, start_idx, end_idx, is_causal],
#             enabled_precisions={torch.float32},
#             disable_tf32=True,
#             use_python_runtime=True,
#             debug=args.debug,
...
#     print_diff(out_static_cache, out_trt, "Prefill TRT")
