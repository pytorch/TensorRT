...
CUDA kernels. The plugin source code is available at:
...
This implementation has been verified with TensorRT-Edge-LLM release 0.4.0.
...
Build the AttentionPlugin shared library following instructions at the GitHub repository above.
...
class SDPAModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.qkv = nn.Linear(
            HIDDEN_DIM, HIDDEN_DIM + 2 * NUM_KV_HEADS * HEAD_DIM, bias=True
        )
...
def test_case(
    name: str, seq_len: int, has_past_context: bool, sdpa_model, trt_model, rope
):
    """
    ...
    if has_past_context:
        past_values = torch.randn(
            BATCH_SIZE, 2, NUM_KV_HEADS, past_len, HEAD_DIM, dtype=DTYPE, device=DEVICE
        )
    ...
    attn_sim = F.cosine_similarity(
        sdpa_out.flatten().float(), trt_out.flatten().float(), dim=0
    ).item()
...
if __name__ == "__main__":
    ...
    initial_seq_len = 16
    x_init = torch.randn(
        BATCH_SIZE, initial_seq_len, HIDDEN_DIM, dtype=DTYPE, device=DEVICE
    )
    ctx_len_init = torch.tensor([initial_seq_len], dtype=torch.int32, device=DEVICE)

    sdpa_kv_multi = torch.zeros(
        BATCH_SIZE,
        2,
        NUM_KV_HEADS,
        KV_CACHE_CAPACITY,
    ...
    with torch.no_grad():
        sdpa_out_init, sdpa_kv_multi = sdpa_model(
            x_init, sdpa_kv_multi, ctx_len_init, rope
        )
    ...
    init_sim = F.cosine_similarity(
        sdpa_out_init.flatten().float(), trt_out_init.flatten().float(), dim=0
    ).item()
    ...
    for gen_step in range(num_gen_tokens):
        ...
        with torch.no_grad():
            sdpa_out_gen, sdpa_kv_multi = sdpa_model(
                x_gen, sdpa_kv_multi, ctx_len_gen, rope
            )
            trt_out_gen, trt_kv_delta = trt_model(
                x_gen, trt_kv_multi, ctx_len_gen, rope
            )
        ...
        gen_sim = F.cosine_similarity(
            sdpa_out_gen.flatten().float(), trt_out_gen.flatten().float(), dim=0
        ).item()
    ...
    results.append(
        (
            all_passed_multi,
            1.0 if all_passed_multi else 0.0,
            1.0 if all_passed_multi else 0.0,
        )
    )
