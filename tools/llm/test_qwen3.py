...
def test_qwen3_model(args):
    ...
    position_ids = (
        torch.arange(input_ids.shape[1], dtype=torch.int64).cuda().unsqueeze(0)
    )
    ...
    ep = torch.export.export(
        model, (input_ids, position_ids), dynamic_shapes=dynamic_shapes
    )
    ...
    print(
        f"Diff b/w pyt and trt: {torch.mean(torch.abs(pyt_output[1] - trt_output[1]))}"
    )
    print(
        f"Diff b/w pyt and trt: {torch.mean(torch.abs(pyt_output[2] - trt_output[2]))}"
    )
    assert torch.allclose(pyt_output[0], trt_output[0], atol=ATOL, rtol=RTOL)
