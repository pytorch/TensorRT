...
DEFAULT_PLUGIN_PATH = "/develop/TensorRT/TensorRT-Edge-LLM-release/build/libNvInfer_edgellm_plugin.so"
...
def convert_attn(ctx: ConversionContext, target, args, kwargs, name):
    ...
    field_list = [
        ...
        trt.PluginField("enable_delta_kv_output", np.array([1], dtype=np.int32), trt.PluginFieldType.INT32),
    ]
...
class PluginAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        kv_cache_start_idx = torch.zeros(batch_size, dtype=torch.int32, device=hidden_states.device)
            
        attn_out, updated_kv = torch.ops.xqa.attn.default(
            qkv, past_key_value, ctx_len, rope_fp32, kv_cache_start_idx,
...
def benchmark_plugin_generation(
    ...
    for _ in range(osl - 1):
        ...
        output = model_func(input_ids_step, position_ids_step, kv_caches, ctx_len_step, **extra_kwargs)
