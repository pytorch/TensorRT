...
parser.add_argument(
    ...
    default="transformer",
    help="Model to use (all or transformer or tokenizer or tokenizer_2 or vae or text_encoder or text_encoder_2)",
)
...
if args.model == "all":
    ...
    model = pipe.transformer
elif args.model == "tokenizer":
    model = pipe.tokenizer
elif args.model == "tokenizer_2":
    model = pipe.tokenizer_2
elif args.model == "vae":
    model = pipe.vae
elif args.model == "text_encoder":
    model = pipe.text_encoder
elif args.model == "text_encoder_2":
...
with torch_tensorrt.logging.debug():
    ...
    trt_ep_path = os.path.join(
        tempfile.gettempdir(),
        f"{args.model}_{args.dtype}_{args.debug}_{args.sdpa}_{args.mha}_trt.ep",
    )
    torch_tensorrt.save(trt_gm, trt_ep_path, inputs=dummy_inputs)
    print(f"TRT module saved to {trt_ep_path}")
